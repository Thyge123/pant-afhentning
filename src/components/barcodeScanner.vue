<template>
    <v-container class="pa-2">
        <v-container class="pa-0" fluid>
            <StreamBarcodeReader @decode="onDecode" @loaded="onLoaded" @result="onResult"></StreamBarcodeReader>
        </v-container>
        <v-container>
            ...
            <v-row v-for="(beverage, index) in sortedBeverages" :key="beverage.id" no-gutters class="ma-1 justify-between">
                ...
                <v-col cols="2" class="d-flex align-center">
                    <p class="text-caption">{{ beverage.isUnregistered ? '-' : beverage.count }}</p>
                </v-col>
                <v-col cols="2" class="d-flex align-center">
...
export default {
    ...
    data() {
        ...
        computed: {
            sortedBeverages() {
                const scannedItems = this.beverages.filter(item => this.scanOrder.includes(item.id));
                const unscannedItems = this.beverages.filter(item => !this.scanOrder.includes(item.id));
        ...
        methods: {
            ...
            onDecode(text) {
                ...
                if (existingItem) {
                    existingItem.count++;
                    existingItem.total = existingItem.count * this.getPantPrice(existingItem.pant);
